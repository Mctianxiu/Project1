...
function image_generating(img_num,shot_num)
    ...
    imwrite(uint16(final_save),['simulated_images/', shot_num,'sim/noisy/noisy_', shot_num,'_', num2str(img_num),'nclst_',num2str(cluster_num) ,'.tif']);

    
    cleansim = E_noise + sim + cleanbkg;
    clean_save = cleansim - 500 + min(min(kernel_noise)) + bkgroundvalue;
    clean_save(clean_save<0) = 0;
    imwrite(uint16(clean_save),['simulated_images/', shot_num,'sim/clean/clean_', shot_num,'_', num2str(img_num),'nclst_',num2str(cluster_num) ,'.tif']);

    
    cleanorg = sim;
    cleanorg_save = cleanorg - 500  + bkgroundvalue;
    imwrite(uint16(cleanorg_save),['simulated_images/', shot_num,'sim/cleanorg/cleanorg_', shot_num,'_', num2str(img_num),'nclst_',num2str(cluster_num) ,'.tif']);

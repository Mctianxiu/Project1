...
def scale_pyramid(img, num_scales):  # torch.Size([8, 3, 256, 256]), num_scales = 4
    scaled_imgs = [img]  # torch.Size([8, 3, 256, 256])

    s = img.size()  # torch.Size([8, 3, 256, 256])
    ...
    for i in range(1, num_scales):
        ...
        nh = h // ratio
        nw = w // ratio
        scaled_img = F.upsample(img, size=(nh, nw), mode='bilinear', align_corners=True)  # torch.Size([8, 3, 128, 128])
        # torch.Size([8, 3, 64, 64]) torch.Size([8, 3, 32, 32])
